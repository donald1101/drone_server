...
public class NioMultiCastDatagramAcceptor extends AbstractIoAcceptor implements DatagramAcceptor, IoProcessor<NioMultiCastSession> {
    /**
     ...
     */
    private final Queue<AbstractIoAcceptor.AcceptorOperationFuture> registerQueue = new ConcurrentLinkedQueue<>();

    private final Queue<AbstractIoAcceptor.AcceptorOperationFuture> cancelQueue = new ConcurrentLinkedQueue<>();

    private final Queue<NioMultiCastSession> flushingSessions = new ConcurrentLinkedQueue<>();
     ...
     */
    public NioMultiCastDatagramAcceptor() {
     ...
     */
    public NioMultiCastDatagramAcceptor(Executor executor) {
     ...
     * Creates a new instance.
     */
    private NioMultiCastDatagramAcceptor(IoSessionConfig sessionConfig, Executor executor) {
    ...
    private int registerHandles() {
        for (; ; ) {
            AbstractIoAcceptor.AcceptorOperationFuture req = registerQueue.poll();
                ...
                if (key.isValid()) {
                    ...
                    if (key.isWritable()) {
                        for (IoSession session : getManagedSessions().values()) {
                            final NioMultiCastSession x = (NioMultiCastSession) session;
    ...
    private boolean scheduleFlush(NioMultiCastSession session) {
    ...
    private IoSession newSessionWithoutLock(SocketAddress remoteAddress, SocketAddress localAddress) throws Exception {
        ...
        synchronized (sessionRecycler) {
            ...
            NioMultiCastSession newSession = newSession(this, handle, remoteAddress);
    ...
    private void flushSessions(long currentTime) {
        for (; ; ) {
            NioMultiCastSession session = flushingSessions.poll();
    ...
    private boolean flush(NioMultiCastSession session, long currentTime) throws Exception {
    ...
    private int unregisterHandles() {
        ...
        for (; ; ) {
            AbstractIoAcceptor.AcceptorOperationFuture request = cancelQueue.poll();
            if (request == null) {
                break;
            }
     ...
     */
    @Override
    public void add(NioMultiCastSession session) {
     ...
     */
    @Override
    public void flush(NioMultiCastSession session) {
    ...
    public TransportMetadata getTransportMetadata() {
        return NioMultiCastDatagramSession.METADATA;
    }
    ...
    protected NioMultiCastSession newSession(IoProcessor<NioMultiCastSession> processor, DatagramChannel handle,
        ...
        NioMultiCastDatagramSession newSession = new NioMultiCastDatagramSession(this, handle, processor, remoteAddress);
    ...
    protected DatagramChannel open(SocketAddress localAddress) throws Exception {
        ...
        try {
            NetworkInterface ni = NetworkInterface.getByInetAddress(InetAddress.getByName(networkInterface));
            NioMultiCastDatagramSessionConfig config = new NioMultiCastDatagramSessionConfig(ch);
            config.setGroup(InetAddress.getByName(group));
            config.setNetworkInterface(ni);
            config.setAll(getSessionConfig());
     ...
     */
    @Override
    public void remove(NioMultiCastSession session) {
    ...
    protected int send(NioMultiCastSession session, IoBuffer buffer, SocketAddress remoteAddress) throws Exception {
    ...
    protected void setInterestedInWrite(NioMultiCastSession session, boolean isInterested) throws Exception {
     ...
     */
    @Override
    public void updateTrafficControl(NioMultiCastSession session) {
        // Nothing to do
    }
     ...
     */
    @Override
    public void write(NioMultiCastSession session, WriteRequest writeRequest) {
